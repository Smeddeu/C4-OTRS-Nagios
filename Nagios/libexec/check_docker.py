...
def make_perfdata(check_dict, uom, perf_min, perf_max):
    ...
    for k, x in check_dict.iteritems():
        scaled_warning = scale_threshold(x.warning_text, 1/float(unit_dict[uom.lower()]))
        scaled_critical = scale_threshold(x.critical_text, 1/float(unit_dict[uom.lower()]))
        temp_string = "%s=%s%s;%s;%s;%s;%s" % (
            x.name, x.value, uom, scaled_warning, scaled_critical, 
            perf_min, perf_max)
...
def process_usage(checks, value):
    ...
    # _short -> short output. _long -> long output.
    labels_short = []
    labels_long = []
    values_short = []
    values_long = []
    ...
    #Account for UoM, if specified
    labels = [check.value for name, check in checks.iteritems()]
    adjusted_short = values_short
    adjusted_long = [check.value for name, check in checks.iteritems()]
    if uom:
        adjusted_short = [val / float(unit_dict[uom.lower()]) for val in adjusted_short]
        adjusted_long = [val / float(unit_dict[uom.lower()]) for val in adjusted_long]
