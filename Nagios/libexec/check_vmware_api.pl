...
use Nagios::Monitoring::Plugin::Functions qw(%STATUS_TEXT);
...
sub main {
	...
	} or Nagios::Monitoring::Plugin::Functions::nagios_exit(UNKNOWN, "Missing perl module VMware::VIRuntime. Download and install \'VMware vSphere SDK for Perl\', available at https://my.vmware.com/group/vmware/downloads\n $perl_module_instructions"); #This is, potentially, a lie. This might just as well fail if a dependency of VMware::VIRuntime is missing (i.e VIRuntime itself requires something which in turn fails).
	...
	foreach my $ref_store (@{$datastore})
	{
		...
		if (!defined($subcommand) || ($name eq $subcommand) || (defined($regexpflag) && $name =~ /$subcommand/))
		{
			...
			if ($store->summary->accessible)
			{
				...
				$res = Nagios::Monitoring::Plugin::Functions::max_state($res, $state);
			...
			if(defined($runtime->healthSystemRuntime))
			{
				...
				if (defined($cpuStatusInfo))
				{
					foreach (@$cpuStatusInfo)
					{
						...
						$res = Nagios::Monitoring::Plugin::Functions::max_state_alt($res, $state);
						if ($state != OK) {
							$AlertCount++;
						} else {
							$OKCount++;
...
sub host_storage_info
{
	...
	else
	{
		...
		foreach my $dev (@{$storage->storageDeviceInfo->hostBusAdapter})
		{
			...
			$state = Nagios::Monitoring::Plugin::Functions::max_state($state, $status);
		}
		$np->add_perfdata(label => "adapters", value => $count, uom => 'units', threshold => $np->threshold);
...
sub vm_disk_io_info
{
	...
	if (defined($subcommand))
	{
		if ($subcommand eq "USAGE")
		{
			$values = return_host_vmware_performance_values($vmname, $defperfargs, 'virtualDisk', ('read.average:*', 'write.average:*'));

			if (defined($values))
			{
				# Even the original value here (metricId 126) was in KBps, not KB
				my $value2 = simplify_number(convert_number($$values[0][0]->value) / 1024);
				my $value3 = simplify_number(convert_number($$values[0][1]->value) / 1024);
				my $value = $value2 + $value3;
				$np->add_perfdata(label => "io_usage", value => $value, uom => '', threshold => $np->threshold);
				$output = "\"$vmname\" io usage=" . $value . " MB/s";
				$res = $np->check_threshold(check => $value);
			}
		}
		elsif ($subcommand eq "READ")
		{
			$values = return_host_vmware_performance_values($vmname, $defperfargs, 'virtualDisk', ('read.average:*'));
			if (defined($values))
			{
				my $value = simplify_number(convert_number($$values[0][0]->value) / 1024);
		...
		{
			$values = return_host_vmware_performance_values($vmname, $defperfargs, 'virtualDisk', ('write.average:*'));
			if (defined($values))
			{
				my $value = simplify_number(convert_number($$values[0][0]->value) / 1024);
	...
	else
	{
		...
		if (defined($values))
		{
			...
			$np->add_perfdata(label => "io_usage", value => $value1, uom => '', threshold => $np->threshold);
			...
			$res = OK;
			$output = "\"$vmname\" io usage=" . $value1 . " MB/s, read=" . $value2 . " MB/s, write=" . $value3 . " MB/s";
		}
	}
		...
		elsif ($subcommand eq "STATUS")
		{
			...
			foreach my $dc (@$dc_views) {
				if (defined($dc->overallStatus))
				{
					...
					$res = Nagios::Monitoring::Plugin::Functions::max_state($res, $status) if (($res != UNKNOWN) || ($status != OK));
				}
				else
				{
					$output .= "Insufficient rights to access " . $dc->name . " status info on the DC, ";
					$res = Nagios::Monitoring::Plugin::Functions::max_state($res, WARNING);
				}
			}
			if ($output) {
				chop($output);
